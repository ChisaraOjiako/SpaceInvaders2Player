...
PLAYER_2 = pygame.image.load(os.path.join("assets", "pixel_ship_yellow.png"))
...
def main():
    ...
    player = Player(100,680)
    player2 = Player2(500,680)
    ...
    def redraw_window():
        WIN.blit(BG, (0,0))
        # Draw text
        lives_label = main_font.render(f"Lives: {lives}",1,(255,87,51))
        level_label = main_font.render(f"Level: {level}",1, (255,87,51))
    ...
    while run:
        ...
        if keys[pygame.K_DOWN] and player2.y+player_vel + player2.get_height() + 20 < HEIGHT: #Down
            player2.y += player_vel
        if keys[pygame.K_j]:
            player2.shoot()
...
def main_menu():
    ...
    while run:
        WIN.blit(BG, (0,0))
        title_label = title_font.render("Click anywhere to begin...", 1, (255,255,255))
        WIN.blit(title_label, (WIDTH/2 -title_label.get_width()/2, 325))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
